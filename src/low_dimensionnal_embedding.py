...
@np.vectorize
def month_to_circle(month: str) -> np.array:
    angle = math.pi * int(month) / 6
...
def get_data_transformers(
    ...
    month_encoder_sin = FunctionTransformer(
        np.vectorize(lambda month: math.sin(math.pi * int(month)) / 6),
        check_inverse=False,
    )
    month_encoder_cos = FunctionTransformer(
        np.vectorize(lambda month: math.cos(math.pi * int(month)) / 6),
        check_inverse=False,
    )
    ...
    x_transformer = ColumnTransformer(
        [
            ...
            ("percentages", percentage_scaler, x_percentage_features),
            ("month_sin", month_encoder_sin, ["month"]),
            ("month_cos", month_encoder_cos, ["month"]),
        ]
    )
...
def load_data(
    ...
    if split_2023:
        data_before_2023 = data[data["date"] < "2023-01"].copy()
        data_2023 = data[data["date"] >= "2023-01"].copy()
...
def load_and_split_train_test(
    ...
    data_train = data_before_2023[data_before_2023["date"].isin(months_train)].copy()
    data_test = data_before_2023[data_before_2023["date"].isin(months_test)].copy()
    link_month_embeddings = defaultdict(list)
    for _, row in data_train.iterrows():
        departure = row["gare_depart"]
        arrival = row["gare_arrivee"]
        link_month_embeddings[departure, arrival].append(
            [
    ...
    link_month_embeddings = {
        key: np.mean(values, axis=0) for key, values in link_month_embeddings.items()
    }
    for i, feature_name in enumerate(
        ...
        data_train[feature_name] = data_train.apply(
            lambda row: link_month_embeddings[row["gare_depart"], row["gare_arrivee"]][
                i
            ],
            axis=1,
        )
        data_test[feature_name] = data_test.apply(
            lambda row: link_month_embeddings[row["gare_depart"], row["gare_arrivee"]][
                i
            ],
            axis=1,
        )
        data_2023[feature_name] = data_2023.apply(
            lambda row: link_month_embeddings[row["gare_depart"], row["gare_arrivee"]][
                i
            ],
            axis=1,
